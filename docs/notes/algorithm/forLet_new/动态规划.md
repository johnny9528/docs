...
设 `dp[i][k]`代表的是数组的前 `i`个划分为 `k`个相邻子数组的最大平均值和。那么一般情况下有：

`dp[i][k] = max(dp[j][k - 1] + avg(j, i))` 其中， `j < i`

意思是，前 `i`个划分为`k`个的最大值等于 前 `j`个划分为 `k - 1`个，加上`j` 到 `i`这一段的平均值， 的最大值。

需要注意一些初始化和特殊情况：

* `dp[i][0]`是没有意义的，因为一个数组不可能划分为0个数组， 因此初始化的时候，需要初始化所有的`dp[i][1]`
* `dp[i][k]` 如果 `i < k`也是没有意义的
* 多次求某个连续段内的平均值， 可以利用前缀和，进行快速计算
...
class Solution {
    public double largestSumOfAverages(int[] nums, int K) {
       ...
       // 初始化，当只划分成1个的时候，因为划分成0个是不符合物理意义的，必须初始化 k = 1
       for (int i = 1; i <= nums.length; i++) {
           dp[i][1] = preSum[i] * 1.0 / i;
       }

       for (int k = 2; k <= K; k++) {
           for (int i = k; i <= nums.length; i++) { // i  需要大于 k， 否则不可能划分成功
               for (int j = 1; j <= i; j++) {
                   double avg = getAverage(preSum, j, i); // j 到 i 这个段内的平均值，利用前缀和快速求得
                   double score = avg + dp[j - 1][k - 1]; // 平均值加上之前的值
                   dp[i][k] = Math.max(dp[i][k], score); // 取最大的
               }
           }
       }
    ...
    // 利用前缀和求某个段的均值
    private double getAverage(int[] preSum, int left, int right) {
        return (preSum[right] - preSum[left - 1]) * 1.0 / (right - left + 1);
    }
}
```
