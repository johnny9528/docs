...
简单递归，用set保存已经访问过的值即可。或者中序遍历得到结果之后，在一个有序的数组中进行目标的查找
...
class Solution {
    ...
    private TreeNode buildTree(int[] nums, int left, int right) {
        ...
        newRoot.left = buildTree(nums, left, index - 1);
        newRoot.right = buildTree(nums, index + 1, right);
        return newRoot;
    }
}
```
