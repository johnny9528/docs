...
如果当前值和上一个值相等，那么将当前值 + 1即可。 为什么是加1， 而不是加上更大的值。 因为如果加上更大的值， 会更大概率对后面的值产生影响。即操作的次数可能会增多。

如果小于之前的一个值(排序之后还小于之前的值的原因是因为 之前一个值可能变大了)
...
class Solution {
    public int minIncrementForUnique(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            if (nums[i] == nums[i - 1]) {
                // 和前一个相等，加1， 为什么是加1， 因为如果变得更大的话，更可能会影响后面的元素
                nums[i] += 1;
                ans += 1;
            } else if (nums[i] < nums[i - 1]) {
                // 因为 nums[i - 1] 可能被改变了， 变大了, 将 nums[i] 变为 nums[i - 1] + 1
...
只要知道每个地方的 左侧的最大值  和 右侧的最小值， 此时只用判断 最大值是否小于等于最小值， 就是满足条件的地方。因此求每个地方的这两个值。
...
class Solution {
    public int partitionDisjoint(int[] nums) {
        ...
        int len = nums.length;
        int[] leftMax = new int[len];   // leftMax[i] 意思是左侧的最大值， 包含这个值本身
        int[] rightMin = new int[len]; // rightMin[i] 意思是右侧的最小值， 不包含这个值本身
...
和寻找最小的K个值， 或者寻找中位数没有任何区别，用一趟快排思路。
...
class Solution {
    public int[][] kClosest(int[][] points, int K) {
        quickSortOnce(points, K, 0, points.length - 1);
    ...
    private void quickSortOnce(int[][] points, int K, int startIndex, int endIndex) {
        if (startIndex >= endIndex) {
            return;
        }

        int base = getLength(points[startIndex]);
        int left = startIndex;
        int right = endIndex;
        while (left < right) {
            while (left < right && getLength(points[right]) >= base) {
                right -= 1;
            }
            swap(points, left, right);
            while (left < right && getLength(points[left]) <= base) {
                left += 1;
            }
            swap(points, left, right);
        }
        ...
        if (left == K) {
            ...
            quickSortOnce(points, K, startIndex, left - 1);
        } else {
            quickSortOnce(points, K, left + 1, endIndex);
        }
...
连续子数组的和，自然而言想到前缀和，用`map`保存前缀和，看是否能匹配即可。
...
class Solution {
    public int subarraysDivByK(int[] A, int K) {
        Map<Integer, Integer> preSumMap = new HashMap<>(); // key 是前缀和(取余), value 是对应的个数
        preSumMap.put(0, 1);
        int ans = 0;
        int preSum = 0;
        for (int i = 0; i < A.length; i++) {
            preSum += A[i];
            preSum = preSum % K;  // 对前缀和取余
            if (preSum < 0) {
                preSum += K;
            }

            if (preSumMap.containsKey(preSum)) {
                ans += preSumMap.get(preSum);
            }
