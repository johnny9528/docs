...
一次遍历，记录当前能够到达的最远的地方，如果最远的地方小于当前索引了，说明走不过去了
...
class Solution {
    public boolean canJump(int[] nums) {
        int rightMost = 0; // 能够到的最远地方
        for (int i = 0; i < nums.length; i++) {
            if (rightMost < i) return false; // 能够到的最远地方，已经比当前索引小，说明走不过去了
            rightMost = Math.max(rightMost, nums[i] + i);// 更新最远地方
            if (rightMost >= nums.length - 1) return true; // 已经超过最大值了，铁定能过去
        }
        return false;
    }
}
